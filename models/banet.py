...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.,
                 proj_drop=0., sr_ratio=1, apply_transform=False):
        super().__init__()
...
def rest_lite(pretrained=False, weight_path=None, **kwargs):
...
class DependencyPath(nn.Module):
    def __init__(self, weight_path=None):
        ...
        self.ResT = rest_lite(pretrained=(weight_path is not None), weight_path=weight_path)
...
def build_model(num_classes=1):
    return BANet(num_classes=num_classes)
